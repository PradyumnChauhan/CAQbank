...
import { adminSignupAction } from '@/app/auth/actions'
...
export default function AdminSignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await adminSignupAction(email, password, fullName)
      if (result.success) {
        // Redirect to login with success message
        router.push('/auth/admin/login?message=Account created! You can now login.')
      } else {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 border border-primary-lighter">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-700 font-medium mb-1">Admin Sign Up</p>
        <p className="text-gray-600 text-sm">Create an admin account</p>
      </div>
      ...
      <p className="mt-6 text-center text-gray-700">
        ...
        <Link href="/auth/admin/login" className="font-medium text-primary hover:text-primary-light transition-colors">
          Sign in here
        </Link>
      </p>
      ...
      <p className="mt-4 text-center text-gray-600 text-sm">
        Student?{' '}
        <Link href="/auth/student/signup" className="font-medium text-gray-800 hover:text-primary transition-colors">
          Signup here
        </Link>
      </p>
      ...
      <div className="mt-6 p-4 bg-amber-50 rounded-lg border border-amber-200">
        <p className="text-xs text-amber-800">
          Admin accounts require approval. Check your email for further instructions.
        </p>
      </div>
    </div>
  )
