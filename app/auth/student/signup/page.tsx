...
import { studentSignupAction } from '@/app/auth/actions'
...
export default function StudentSignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await studentSignupAction(email, password, fullName)
      if (result.success) {
        // Redirect to login with success message
        router.push('/auth/student/login?message=Account created! You can now login.')
      } else {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 border border-primary-lighter">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-700 font-medium mb-1">Student Sign Up</p>
      ...
      <p className="mt-6 text-center text-gray-700">
        ...
        <Link href="/auth/student/login" className="font-medium text-primary hover:text-primary-light transition-colors">
          Sign in here
        </Link>
      </p>
      ...
      <p className="mt-4 text-center text-gray-600 text-sm">
        Admin?{' '}
        <Link href="/auth/admin/signup" className="font-medium text-gray-800 hover:text-primary transition-colors">
          Signup here
        </Link>
      </p>
